 ...
 * @version 1.0
 */
public class ASN1IntegerShortTest extends CustomTestCase {

	private final short dataSet[] = new short[]{0, 1, -1, Short.MAX_VALUE, Short.MIN_VALUE};
...
//		System.out.println(Utils.toHexString(data));
		...
		for (int i = 0; i < dataSet.length; i++) {
			tmp = schema.read(bis);
			assertNotNull(tmp);
			assertEquals(tmp.getClass(), Short.class);
			value = (Short) tmp;
			assertTrue(value == dataSet[i]);
		}
